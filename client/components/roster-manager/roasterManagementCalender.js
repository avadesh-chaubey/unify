...
function RoasterCalender(props) {
  ...
  function checkSlotOnselectedday(date) {
    ...
    axios
      ...
      .then((response) => {
        console.log("viewSlot", response);
        availSlot = response.data.data[0].availableSlotsList;
        timeData.map((item) => {
          let obj = {};
        ...
        timeData.map((item) => {
          if (availSlot[item.value] === "blocked") {
            blockedArr.push(item.value);
          }
        });
        // if (temp.length > 0) {
        setSlotSelectedDay(showTimeSlot(temp, bookedArr, blockedArr));
        // }
        setLoader(false);
      })
      .catch((error) => {
  ...
  function changeTimeSlotDay(day) {
    if (responseData.id != undefined) {
      ...
      // } else
      {
        if (
          day == 1 &&
          responseData.mondayAvailableSlotList &&
          responseData.mondayAvailableSlotList.length > 0
        ) {
          setSlotSelectedDay(
        ...
        if (
          day == 2 &&
          responseData.tuesdayAvailableSlotList &&
          responseData.tuesdayAvailableSlotList.length > 0
        ) {
          setSlotSelectedDay(
        ...
        if (
          day == 3 &&
          responseData.wednesdayAvailableSlotList &&
          responseData.wednesdayAvailableSlotList.length > 0
        ) {
          setSlotSelectedDay(
        ...
        if (
          day == 4 &&
          responseData.thursdayAvailableSlotList &&
          responseData.thursdayAvailableSlotList.length > 0
        ) {
          setSlotSelectedDay(
        ...
        if (
          day == 5 &&
          responseData.fridayAvailableSlotList &&
          responseData.fridayAvailableSlotList.length > 0
        ) {
          setSlotSelectedDay(
        ...
        if (
          day == 6 &&
          responseData.saturdayAvailableSlotList &&
          responseData.saturdayAvailableSlotList.length > 0
        ) {
          setSlotSelectedDay(
        ...
        if (
          day == 0 &&
          responseData.sundayAvailableSlotList &&
          responseData.sundayAvailableSlotList.length > 0
        ) {
          setSlotSelectedDay(
  ...
  function showTimeSlot(arr, bookedArr = [], blockedArr = []) {
    let newTimeSlot = [];
    ...
    if (blockedArr.length) {
      //  Iterate element for booked time slot
      const blockedTimeSlot = newTimeSlot.map((i) => {
        const b1 = blockedArr.filter((j) => j === i.value);
  ...
  function fetchSlotFromArr(arr) {
    ...
    let temp = [];
    const filterSelectedSlot = arr.filter(
      (i) => i.selected === false || i.booked
    );
  ...
  const saveSlotSpecDay = () => {
    ...
    axios
      ...
      .catch((error) => {
        ...
        const slotErrMsg =
          "Cannot remove slots with booked appointment. Please reschedule and try again.";
  ...
  const getLeaveDetails = (startDate, endDate) => {
    ...
    axios
      .post(config.API_URL + "/api/appointment/viewslots", obj, { headers })
      .then((response) => {
        console.log("2 view slot", response.data);
        let tempArr = [];
  ...
  const onSlotBtnClick = (i, item) => {
    ...
    if (!temptimeSlot[i].booked && !temptimeSlot[i].blocked) {
  ...
  return (
    <>
      ...
      <div className="rightView calender-div">
        <Grid container spacing={3} className="app-calendar-slot">
          <Grid item xs={6}>
            <div className="calander calendar-main">
              <Calendar
              ...
              {slotSelectedDay.length ? (
                <div className="timeSlots">
                  <div className="addNewSlot">
                    <div className="apmtTimeSlot app-time-slot-2">
                      {slotSelectedDay.map((item, index) => {
                        return (
                          <div
                            ...
                            } ${item.blocked ? "blocked-time-slot" : ""}`}
                            key={index}
