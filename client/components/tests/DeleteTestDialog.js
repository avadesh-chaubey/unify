...
const DeleteTestDialog = (params) => {
  ...
  const removeMedicine = (e) => {
    ...
    axios.get(`${config.API_URL}/api/utility/deletediagnostictest/${id}`, { headers })
      .then(res => {
        setCharValue("")
        closeDialog();
        setMessage({
          message: "Deleted Test Record Successfully"
        });
      })
      .catch(err => setMessage({
        message: "Error occurred while deleting Test Record ",
        type: 'error'
      }));
  };
