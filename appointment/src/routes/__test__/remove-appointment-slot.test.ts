...
it('has a route handler listening to /api/appointment/removeslots for post requests', async () => {
  const response = await request(app)
    .post('/api/appointment/removeslots')
    .send({});
...
it('returns an 200 if an valid Date is provided', async () => {
  ...
  await request(app)
    ...
    .send({
      consultantId: id,
      appointmentDate: appointmentDate,
      removeSlotList: availableSlots
    })
    .expect(200);

  const slott = await AppointmentConfig.find({});
  expect(slott.length).toEqual(1);
  const arrayy = [...slott[0].availableSlots];
  expect(arrayy[2]).toEqual(SlotAvailability.Unavailable);
  expect(arrayy[4]).toEqual(SlotAvailability.Unavailable);
  expect(arrayy[6]).toEqual(SlotAvailability.Unavailable);
});
