import { Listener, AddConsultantAppointmentSlotsEvent, Subjects, SlotAvailability, UserType } from '@unifycaredigital/aem';
import { Message } from 'node-nats-streaming';
import { consultantAppointmentSlotGroupName } from './queue-group-name';
...
export class AddConsultantAppointmentSlotsListener extends Listener<AddConsultantAppointmentSlotsEvent> {
  subject: Subjects.AddConsultantAppointmentSlots = Subjects.AddConsultantAppointmentSlots;
  queueGroupName = consultantAppointmentSlotGroupName;

  async onMessage(data: AddConsultantAppointmentSlotsEvent['data'], msg: Message) {
    console.log('AddConsultantAppointmentSlotsEvent Received for date: ', data.message);

    //Algo
    //1. Calculate nextDate = today + 31 days
    const day = moment().utcOffset(330).add(FUTURE_APPOINTMENT_ALLOWED_IN_DAYS, 'days').day();
    const date = moment().utcOffset(330).add(FUTURE_APPOINTMENT_ALLOWED_IN_DAYS, 'days').format('YYYY-MM-DD');
    console.log('AddConsultantAppointmentSlotsEvent day: ' + day + ' date:' + date);
      ...
      if (consultantList[i].userType === UserType.PhysicianAssistant) {
        ...
        if (existingAssistantRoster && finalAvailableAssistantList) {
          ...
          if (!iAmFirstAssistant) {
            ...
            console.log('assistant slot update !iAmFirstAssistant finalAvailableAssistantList' + JSON.stringify(finalAvailableAssistantList));
          } else {
            const roster = {
              physicianAssistantId: assistantId!,
              dateInYYYYMMDD: date,
            ...
            console.log('assistant slot update iAmFirstAssistant: ');
          }
        } else {
          console.log('assistant slot NO existingAssistantRoster !!!');
        }
