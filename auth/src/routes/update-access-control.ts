...
import { BadRequestError, validateRequest, ApiResponse } from '@unifycaredigital/aem';
import { AccessControl } from '../models/access-control';
import { natsWrapper } from '../nats-wrapper';
//import { AccessControlUpdatedPublisher } from '../events/publishers/access-control-updated-publisher';

const router = express.Router();

router.put(
	'/api/users/updateaccesscontrol/:id',
	[
	],
	validateRequest,
	async (req: Request, res: Response) => {
		...
		const accessContorl = await AccessControl.findById(accessId);
		if (!accessContorl) {
			throw new BadRequestError("AccessControl not exists");
		}
		accessContorl.set({
		...
		console.log('accessContorl-->', accessId);
		await accessContorl.save();

		//new AccessControlUpdatedPublisher(natsWrapper.client).publish({
...
export { router as updateAccessControlRouter };
