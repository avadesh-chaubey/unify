...
import { appointmentRazorpay } from '../razorpay';
...
router.post(
    ...
    async (req: Request, res: Response) => {
        ...
        if (existingOrder.orderType === OrderType.FreeAppointment) {
            ...
            try {
                await appointmentRazorpay.orders.create(options, function (err: any, order: any) {
                    if (err) {
                        req.errorlog(err);
                        console.log(err);
...
export { router as payAppointmentOrderRouter };
